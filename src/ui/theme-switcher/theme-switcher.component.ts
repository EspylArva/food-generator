...
private readonly STORAGE_KEY = 'themeSwitcherState';
  ...
  themeState = signal<ThemeState>({darkTheme: false, preset: 'Aura', primary: 'blue', surface: 'light'});
  ...
  primaryColors : any = computed(() => {
    ...
    const presetPalette = presets[preset as keyof typeof presets].primitive;
    const colors = [
      'emerald',
      'green',
      'lime',
    ...
    colors.forEach((color) => {
      if (presetPalette && presetPalette[color as keyof typeof presetPalette]) {
        palettes.push({
          name: color,
